...
class Studentupdate(BoxLayout):
   ...
   def choose(self,instance):
        fct=self.ids.faculity.text
        level=self.ids.level.text
        try:
            if level=="Bachelor" and fct=="Engineering":
                self.ids.program.values=("Computer","Civil","Electrical","Electronic Communcation")
            elif level=="Bachelor" and fct=="IT":
                self.ids.program.values=("BCA","BIT","AI","PGDCA")
            elif level=="Master" and fct=="IT":
                self.ids.program.values=("MCA","MIT")
            elif level=="Master" and fct=="Engineering":
                self.ids.program.values=("Msc EarthEquake","MSC Construction Management","Electrical Power Engineering","MSC ISE","MSC EM",)
            else:
                self.ids.program.values          
        except Exception as e:
            print(e)
    ...
    def submit(self,instance,rt):
        try:
            ...
            popup = Popup(title="Message",
                      ...
                      size=(500, 500))
            popup.open()
    def back(self,instance,rt):
        rt.ids.sname.text= ""
        rt.ids.sid.text=""
        rt.ids.sprogram.text=""
        rt.ids.sfact.text= ""
        rt.ids.sbatch.text=""
        rt.ids.slevel.text= ""
        rt.ids.smobileno.text=""
    ...
    def submit(self,instance,rt):
        try:
            ...
            if bookd[9]>0 and st[11]>st[12] and st[10]=="Eligible":
                ...
                popup.open()
                cursor = conn.cursor()
                rt.ids.blname.text="name of book"
                rt.ids.blid.text="book id"
                rt.ids.slname.text="name of student"
                rt.ids.slid.text="student id"
                rt.ids.isdate.text="2060/10/10"
...
class Studentdetials(BoxLayout):
    def choose(self,instance):
        fct=self.ids.sfact.text
        level=self.ids.slevel.text
        print(self.ids.sfact.text,"\n",self.ids.slevel.text)
        try:
            if level=="Bachelor" and fct=="Engineering":
                self.ids.sprogram.values=("Computer","Civil","Electrical","Electronic Communcation")
            elif level=="Bachelor" and fct=="IT":
                self.ids.sprogram.values=("BCA","BIT","AI""PGDCA",)
            elif level=="Master" and fct=="IT":
                self.ids.sprogram.values=("MCA","MIT")
            elif level=="Master" and fct=="Engineering":
                self.ids.sprogram.values=("Msc EarthEquake","MSC Construction Management","Electrical Power Engineering","MSC ISE","MSC EM",)
            else:
                self.ids.sprogram.values          
        except Exception as e:
            print(e)
